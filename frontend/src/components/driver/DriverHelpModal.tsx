...
const DriverHelpModal = ({ onClose, batchId, driverId }: DriverHelpModalProps) => {
    const openWhatsApp = (category: string) => {
        const msg = buildHelpMessage(category, { batchId, driverId });
                ...
                <div className="space-y-3 text-sm">
                    {CATEGORIES.map(({ label, description }) => (
                        <button
                            ...
                            className="w-full text-left rounded-xl border border-border p-4 hover:border-success/30 transition-colors"
                        >
