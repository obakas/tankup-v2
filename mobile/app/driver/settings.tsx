...
export default function DriverNotificationSettings() {
  ...
  useEffect(() => {
    ...
    getNotificationPreferences("driver", actor_id)
  ...
  async function handleToggle(key: string, value: boolean) {
    ...
    setPrefs(optimistic);
    try {
      const result = await updateNotificationPreferences("driver", actor_id, { [key]: value });
  ...
  const categories = CATEGORIES.driver;
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.background }}>
      ...
      >
        ...
        <View style={{ backgroundColor: theme.successSoft, borderRadius: 8, padding: 6 }}>
          <Bell color={theme.success} size={16} />
        </View>
      ...
      {loading ? (
        ...
        <View className="flex-1 items-center justify-center p-6">
          <Text style={{ color: theme.mutedForeground }} className="text-center text-sm">
            Log in as a driver to manage notification preferences.
          </Text>
        </View>
      ) : (
        <ScrollView
